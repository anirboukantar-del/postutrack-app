...
export function A4PreviewWrapper({ children, scaleMode = 'auto', className = '', isMultiPage = false }) {
...
export function ResumeRenderer({
  ...
  // Density spacing scales with balanced top & bottom margins
  const densityStyles = {
    compact: {
    ...
    normal: {
      ...
      sectionGap: 'mb-2',
      bodyText: 'text-[11.5px] sm:text-[12px] leading-relaxed',
  ...
  const renderTemplateContent = () => {
    ...
    if (template === 'rendercv') {
      return (
        <div 
          id="cv-render" 
          className={`bg-white border border-gray-300 ${densityStyles.padding} w-[210mm] min-w-[210mm] max-w-[210mm] ${
            isMultiPage ? 'multi-page-cv min-h-[297mm] h-auto' : 'single-page-cv min-h-[297mm]'
          } box-border shadow-md text-black font-serif select-text print:border-none print:shadow-none print:p-8 print:m-0 flex flex-col gap-3.5`}
          style={{ 
            userSelect: 'text', 
        ...
        >
          {/* Header */}
          <div className="text-center mb-0.5 flex items-center justify-center gap-4 cv-section">
            {hasPhoto && (
              <img 
                src={photoUrl} 
                alt={displayName} 
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-normal mb-1">{displayName}</h1>
              <div className="text-[11.5px] flex justify-center items-center gap-3 flex-wrap text-gray-700">
          ...
          {cv.summary && (
            <div className="cv-section">
              <p className="text-[12px] leading-relaxed text-justify">{cv.summary}</p>
            </div>
          )}
          ...
          {cv.experiences && cv.experiences.length > 0 && (
            <div className="cv-section">
              <h3 className="text-[13px] font-bold uppercase border-b-2 border-black mb-1.5 pb-0.5 tracking-wider">
                {t?.experienceTitle || 'Expérience Professionnelle'}
              </h3>
              <div className="flex flex-col gap-2.5">
                {cv.experiences.map((exp, idx) => (
                  <div key={idx} className="cv-section-item cv-experience-item">
                    <div className="flex justify-between items-baseline mb-0.5">
                      <div className="font-bold text-[12.5px]">{exp.company}</div>
                      <div className="text-[11.5px] text-gray-700">{exp.period}</div>
                    </div>
          ...
          {structuredSkills.length > 0 && (
            <div className="cv-section">
              ...
              <div className="flex flex-col gap-1.5 text-[11.5px] w-full">
                {structuredSkills.map((group, idx) => (
                  <div key={idx} className="flex flex-row items-start cv-section-item cv-skills-item">
                    <h4 className="font-bold uppercase w-[150px] shrink-0 text-right pr-3 leading-tight pt-[1px]">{group.category}</h4>
    ...
    if (template === 'azurill') {
      return (
        <div 
          id="cv-render" 
          className={`bg-white border border-gray-200 ${densityStyles.padding} w-[210mm] min-w-[210mm] max-w-[210mm] ${
            isMultiPage ? 'multi-page-cv min-h-[297mm] h-auto' : 'single-page-cv min-h-[297mm]'
          } box-border shadow-md text-slate-800 font-sans select-text print:border-none print:shadow-none print:p-8 print:m-0 flex flex-col ${densityStyles.gap}`}
          style={{ 
            userSelect: 'text', 
            WebkitUserSelect: 'text',
            minHeight: '297mm',
        ...
        >
          {/* Header */}
          <div className="border-b pb-2.5 flex justify-between items-center gap-4 cv-section" style={{ borderColor: `${accentColorHex}30` }}>
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight" style={{ color: accentColorHex }}>
                {displayName}
              </h1>
              
              <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1 mt-1.5 text-[11px] sm:text-[11.5px] text-slate-600">
                {displayLocation && (
                  <span className="flex items-center gap-1">
                    <MapPin size={12} style={{ color: accentColorHex }} />
                    {displayLocation}
                  </span>
                )}
                {displayEmail && (
                  <span className="flex items-center gap-1">
                    <Mail size={12} style={{ color: accentColorHex }} />
                    {displayEmail}
                  </span>
                )}
                {displayPhone && (
                  <span className="flex items-center gap-1">
                    <Phone size={12} style={{ color: accentColorHex }} />
                    {displayPhone}
                  </span>
                )}
                {displayWebsite && (
                  <span className="flex items-center gap-1">
                    <Globe size={12} style={{ color: accentColorHex }} />
                    {displayWebsite}
                  </span>
                )}
              </div>
          ...
          {cv.experiences && cv.experiences.length > 0 && (
            <div className="cv-section">
              <div className="flex items-center gap-2 pb-1 mb-2 border-b-2" style={{ borderColor: accentColorHex }}>
                <Briefcase size={14} style={{ color: accentColorHex }} />
              ...
              <div className={densityStyles.itemGap}>
                {cv.experiences.map((exp, idx) => (
                  <div key={idx} className="relative pl-3 border-l-2 cv-section-item cv-experience-item" style={{ borderColor: `${accentColorHex}40` }}>
                    <div className="flex justify-between items-baseline flex-wrap gap-1 mb-0.5">
                      <span className="font-bold text-[12.5px] text-slate-900">{exp.company}</span>
                      <span className="text-[11px] font-semibold" style={{ color: accentColorHex }}>{exp.period}</span>
                    </div>
          ...
          {cv.education && cv.education.length > 0 && (
            <div className="cv-section">
              <div className="flex items-center gap-2 pb-1 mb-2 border-b-2" style={{ borderColor: accentColorHex }}>
                <GraduationCap size={14} style={{ color: accentColorHex }} />
                <h3 className={`${densityStyles.headingText} font-bold uppercase tracking-wider text-slate-900`}>
                  {t?.educationTitle || 'Formation'}
                </h3>
              </div>
          ...
          {structuredSkills.length > 0 && (
            <div className="cv-section">
              <div className="flex items-center gap-2 pb-1 mb-2 border-b-2" style={{ borderColor: accentColorHex }}>
                <Code size={14} style={{ color: accentColorHex }} />
                <h3 className={`${densityStyles.headingText} font-bold uppercase tracking-wider text-slate-900`}>
                  {t?.skillsTitle || 'Compétences'}
                </h3>
              </div>
              ...
              <div className="space-y-1.5">
                {structuredSkills.map((group, idx) => (
                  <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 cv-section-item cv-skills-item">
                    <span className="text-[11px] font-bold text-slate-700 uppercase w-32 shrink-0">{group.category} :</span>
    ...
    if (template === 'bronzor') {
      return (
        <div 
          id="cv-render" 
          className={`bg-white border border-gray-200 w-[210mm] min-w-[210mm] max-w-[210mm] ${
            isMultiPage ? 'multi-page-cv min-h-[297mm] h-auto' : 'single-page-cv min-h-[297mm]'
          } box-border shadow-md text-slate-800 font-sans select-text print:border-none print:shadow-none print:p-0 print:m-0 flex flex-row`}
          style={{ 
            userSelect: 'text', 
            WebkitUserSelect: 'text',
            minHeight: '297mm',
        ...
        >
          {/* Left Sidebar (~33% width) with balanced top and bottom padding */}
          <div 
            className="w-[33%] p-4 sm:p-5 pb-8 sm:pb-10 flex flex-col gap-3.5 text-slate-800 shrink-0 border-r"
            style={{ 
          ...
          >
            ...
            {hasPhoto && (
              <div className="flex flex-col items-center text-center pb-2 border-b cv-section" style={{ borderColor: `${accentColorHex}25` }}>
                <img
                  src={photoUrl}
                  alt={displayName}
            ...
            {/* Contact Details */}
            <div className="space-y-2 text-[11px] cv-section">
              <h4 className="text-[10.5px] font-bold uppercase tracking-wider text-slate-500 mb-1 flex items-center gap-1.5">
                <User size={12} style={{ color: accentColorHex }} />
                Contact
              </h4>
              {displayLocation && (
                <div className="flex items-start gap-1.5">
                  <MapPin size={12} className="shrink-0 mt-0.5" style={{ color: accentColorHex }} />
                  <span className="text-slate-700 break-words">{displayLocation}</span>
                </div>
              )}
              {displayEmail && (
                <div className="flex items-start gap-1.5">
                  <Mail size={12} className="shrink-0 mt-0.5" style={{ color: accentColorHex }} />
                  <span className="text-slate-700 break-all">{displayEmail}</span>
                </div>
              )}
              {displayPhone && (
                <div className="flex items-start gap-1.5">
                  <Phone size={12} className="shrink-0 mt-0.5" style={{ color: accentColorHex }} />
                  <span className="text-slate-700">{displayPhone}</span>
                </div>
              )}
              {displayWebsite && (
                <div className="flex items-start gap-1.5">
                  <Globe size={12} className="shrink-0 mt-0.5" style={{ color: accentColorHex }} />
            ...
            {/* Skills in Sidebar */}
            {structuredSkills.length > 0 && (
              <div className="space-y-2 pt-2 border-t cv-section" style={{ borderColor: `${accentColorHex}20` }}>
                <h4 className="text-[10.5px] font-bold uppercase tracking-wider text-slate-500 mb-1 flex items-center gap-1.5">
                  <Code size={12} style={{ color: accentColorHex }} />
                  {t?.skillsTitle || 'Compétences'}
                </h4>
                {structuredSkills.map((group, idx) => (
                  <div key={idx} className="space-y-0.5 cv-section-item cv-skills-item">
                    <div className="text-[10.5px] font-bold text-slate-800 uppercase">{group.category}</div>
                    <div className="flex flex-wrap gap-1">
                      {group.items?.map((item, i) => (
                        <span 
                          key={i} 
                          className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-white text-slate-800 border"
          ...
          {/* Right Main Content (~67% width) with balanced top and bottom padding */}
          <div className={`w-[67%] ${densityStyles.padding} pb-8 sm:pb-10 flex flex-col ${densityStyles.gap}`}>
            {/* Main Name & Title */}
            <div className="cv-section">
              <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
                {displayName}
              </h1>
              <div className="w-10 h-1 rounded-full mt-1.5" style={{ backgroundColor: accentColorHex }} />
            </div>

            {/* Summary */}
            {cv.summary && (
              <div 
                className="p-2.5 rounded-r-xl border-l-4 bg-slate-50/80 cv-section" 
                style={{ borderColor: accentColorHex }}
              >
                <p className={`${densityStyles.bodyText} text-slate-700 leading-relaxed text-justify`}>
                  {cv.summary}
            ...
            {cv.experiences && cv.experiences.length > 0 && (
              <div className="cv-section">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-900 flex items-center gap-2 pb-1 mb-2.5 border-b" style={{ borderColor: `${accentColorHex}30` }}>
                  <Briefcase size={14} style={{ color: accentColorHex }} />
                  {t?.experienceTitle || 'Expérience Professionnelle'}
                </h3>
                
                <div className="space-y-2.5 relative pl-3.5 border-l-2" style={{ borderColor: `${accentColorHex}40` }}>
                  {cv.experiences.map((exp, idx) => (
                    <div key={idx} className="relative cv-section-item cv-experience-item">
                      {/* Node dot on timeline */}
                      <div 
                        className="absolute -left-[19px] top-1 w-2 h-2 rounded-full border-2 bg-white" 
                        style={{ borderColor: accentColorHex }} 
                      />
                      <div className="flex justify-between items-baseline flex-wrap gap-1 mb-0.5">
            ...
            {cv.education && cv.education.length > 0 && (
              <div className="cv-section">
                ...
                <div className="space-y-2 relative pl-3.5 border-l-2" style={{ borderColor: `${accentColorHex}40` }}>
                  {cv.education.map((edu, idx) => (
                    <div key={idx} className="relative cv-section-item cv-education-item">
                      <div 
                        className="absolute -left-[19px] top-1 w-2 h-2 rounded-full border-2 bg-white" 
                        style={{ borderColor: accentColorHex }} 
                      />
    ...
    return (
      <div 
        ...
        className={`bg-white border border-gray-200 ${densityStyles.padding} w-[210mm] min-w-[210mm] max-w-[210mm] ${
          isMultiPage ? 'multi-page-cv min-h-[297mm] h-auto' : 'single-page-cv min-h-[297mm]'
        } box-border shadow-md text-slate-900 font-sans select-text print:border-none print:shadow-none print:p-8 print:m-0 flex flex-col ${densityStyles.gap}`}
        style={{ 
          userSelect: 'text', 
          WebkitUserSelect: 'text',
          minHeight: '297mm',
      ...
      >
        {/* Executive Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2.5 pb-2.5 border-b-2 border-slate-900 cv-section">
          ...
          <div className="flex flex-wrap sm:flex-col items-start sm:items-end gap-1 text-[11px] text-slate-600">
            {displayEmail && <span>{displayEmail}</span>}
            {displayPhone && <span>{displayPhone}</span>}
            {displayWebsite && <span>{displayWebsite}</span>}
          </div>
        ...
        {cv.experiences && cv.experiences.length > 0 && (
          <div className="cv-section">
            <h3 className="text-[11.5px] font-bold uppercase tracking-widest text-slate-400 pb-1 mb-1.5 border-b border-slate-200">
              {t?.experienceTitle || 'Expérience Professionnelle'}
            </h3>
            <div className={densityStyles.itemGap}>
              {cv.experiences.map((exp, idx) => (
                <div key={idx} className="cv-section-item cv-experience-item">
                  <div className="flex justify-between items-baseline flex-wrap gap-1 mb-0.5">
                    <div className="font-bold text-[12.5px] text-slate-900">
        ...
        {structuredSkills.length > 0 && (
          <div className="cv-section">
            ...
            <div className="space-y-1 text-[11.5px]">
              {structuredSkills.map((group, idx) => (
                <div key={idx} className="flex flex-row items-baseline gap-2 cv-section-item cv-skills-item">
                  <span className="font-bold text-[11px] uppercase tracking-wider text-slate-700 w-32 shrink-0">
                    {group.category}
                  </span>
  ...
  return (
    <A4PreviewWrapper scaleMode={scaleMode} isMultiPage={isMultiPage}>
